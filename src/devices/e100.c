...
static void init_tx_descs(void) {
  ...
  ASSERT( ((uintptr_t)tx_descs & 0xF) == 0 ); // is 16 byte aligned
  ...
  for ( int i = 0; i < TX_DESCS_SZ; ++i ) {
    ...
    // is this space not mapped?
    tx_descs[i].status |= 0x1;
    // ensure that packets are in kernel space
    ASSERT(is_kernel_vaddr(&packets[i]));

    tx_descs[i].addr = (uintptr_t)&packets[i];
...
void e100_transmit(void * data, uint16_t sz) {
  ...
  // set the addr
  void * addr = (void *)&packets[tx_descs_tail];
  ASSERT((uintptr_t)packets[tx_descs_tail] == tx_desc->addr);
  memcpy(addr,data,sz);
  // set the length
  tx_desc->length = sz;
  // set the cso
  ...
  tx_descs_tail &= (TX_DESCS_SZ-1); // like mod 64

  // check if descriptor has been sent
  int j = 0;
  while ( (tx_descs[tx_descs_tail-1].status & 0x1) == 0 ) {
    printf("waiting... %d\n",j);
    ++j;
  }
