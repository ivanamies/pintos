...
// palloc is broken
// use a stack allocator instead

typedef struct stack_allocator {
  struct lock lk;
  uint8_t * pages; // use a 1 byte type
  int page_num;
} stack_allocator_t;

stack_allocator_t kernel_mem, user_mem;
int stack_allocator_max_pages = 150; // you probably can't add more stuff here
...
void free_pages_from_stack_allocator(UNUSED int user, UNUSED void * p) {
